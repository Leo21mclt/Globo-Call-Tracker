/**
 * Injected into globo.na.deputy.com to sync shifts
 */

chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'EXTRACT_DEPUTY_DATA') {
    syncShiftsWithRetry()
  ...
  if (request.action === 'DEPUTY_NEXT_WEEK') {
    ...
    // Grab the current anchor date before clicking next
    let oldAnchor = null;
    const days = document.querySelectorAll('li.m-myWeek-item');
    for (let i = 0; i < days.length; i++) {
      const avatar = days[i].querySelector('.my-week-day__avatars .js-myWeek-avatar');
      if (avatar && avatar.getAttribute('data-my-week-day')) {
        oldAnchor = avatar.getAttribute('data-my-week-day');
        break;
      }
    }
    ...
    waitForDOMSettle(oldAnchor)
      .then(() => syncShiftsWithRetry())
      .then(shifts => sendResponse({ success: true, shifts }))
      .catch(err => sendResponse({ success: false, error: err.message }));
...
function waitForDOMSettle(oldAnchor, settleTimeMs = 800, maxWaitMs = 15000) {
  return new Promise((resolve) => {
    ...
    const checkState = () => {
      ...
      for (let i = 0; i < days.length; i++) {
        const avatar = days[i].querySelector('.my-week-day__avatars .js-myWeek-avatar');
        if (avatar && avatar.getAttribute('data-my-week-day')) {
          currentAnchor = avatar.getAttribute('data-my-week-day');
          break;
        }
      }

      if (!isLoading && oldAnchor && currentAnchor && currentAnchor !== oldAnchor) {
        finish();
      }
    };
    ...
    const resetTimer = () => {
      clearTimeout(settleTimer);
      settleTimer = setTimeout(checkState, settleTimeMs);
    };
    ...
    maxWaitTimer = setTimeout(finish, maxWaitMs);
    resetTimer();
  });
}

function syncShiftsWithRetry(maxWaitMs = 5000) {
...
function parseShiftTimeStr(str) {
  // Clean up periods in am/pm (e.g. a.m. -> am, p.m. -> pm)
  const cleanStr = str.replace(/\./g, '').toLowerCase().trim();
  
  // Try matching 12-hour format first
  const regex12 = /(\d{1,2})(?::(\d{2}))?\s*(am|pm)\s*-\s*(\d{1,2})(?::(\d{2}))?\s*(am|pm)/i;
  ...
  if (match12) {
    ...
    return {
      start: to24Hour(match12[1], match12[2], match12[3]),
      end: to24Hour(match12[4], match12[5], match12[6])
    };
  }
  
  // Try matching 24-hour format (e.g. 08:00 - 12:00 or 8 - 12)
  const regex24 = /(\d{1,2})(?::(\d{2}))?\s*-\s*(\d{1,2})(?::(\d{2}))?/i;
  const match24 = cleanStr.match(regex24);
  if (match24) {
    function padTime(hourStr, minuteStr) {
      let h = parseInt(hourStr, 10);
      const m = parseInt(minuteStr || "0", 10);
      return `${h.toString().padStart(2, '0')}:${m.toString().padStart(2, '0')}`;
    }
    return {
      start: padTime(match24[1], match24[2]),
      end: padTime(match24[3], match24[4])
    };
  }
